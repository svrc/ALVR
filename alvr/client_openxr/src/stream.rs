...
impl StreamContext {
    ...
    pub fn render(
        ...
        /*let mut current_headset_pose = Pose{orientation: view_params[0].pose.orientation, position: (view_params[0].pose.position + view_params[1].pose.position) * 0.5};
        if let Ok((head_location, head_velocity)) = self.view_reference_space.as_ref()
            .relate(self.stage_reference_space.as_ref(), xr_vsync_time) {
                if head_location
                    .location_flags
                    .contains(xr::SpaceLocationFlags::ORIENTATION_VALID)
                {
                    current_headset_pose = crate::from_xr_pose(head_location.pose);
                }
        }*/

        let render_pose_l = current_headset_views[0].pose;
        let render_pose_r = current_headset_views[1].pose;
        let render_fov_l = current_headset_views[0].fov;
        let render_fov_r = current_headset_views[1].fov;
        let frame_fov_l = crate::to_xr_fov(view_params[0].fov);
        let frame_fov_r = crate::to_xr_fov(view_params[1].fov);

        let current_headset_pose_l = crate::from_xr_pose(current_headset_views[0].pose);
        let frame_headset_pose_l = view_params[0].pose;
        let current_headset_pose_r = crate::from_xr_pose(current_headset_views[1].pose);
        let frame_headset_pose_r = view_params[1].pose;

        /*let render_pose_l = crate::to_xr_pose(view_params[0].pose);
        let render_pose_r = crate::to_xr_pose(view_params[1].pose);
        let render_fov_l = crate::to_xr_fov(view_params[0].fov);
        let render_fov_r = crate::to_xr_fov(view_params[1].fov);
        let frame_fov_l = crate::to_xr_fov(view_params[0].fov);
        let frame_fov_r = crate::to_xr_fov(view_params[1].fov);

        let current_headset_pose_l = view_params[0].pose;
        let frame_headset_pose_l = view_params[0].pose;
        let current_headset_pose_r = view_params[1].pose;
        let frame_headset_pose_r = view_params[1].pose;*/
