...
use std::{path::Path, rc::Rc, mem, mem::MaybeUninit, slice, sync::Arc, thread, time::Duration};
...
extern crate libc;
#[cfg(target_os = "android")]
use libc::{c_char, c_int, dlsym, dlopen, RTLD_DEFAULT, RTLD_NOW};
...
impl EyeTrackerSocial {
    pub fn new<G>(session: &xr::Session<G>) -> xr::Result<Self> {
        ...
        let mut fn_GetEyeDataET: Option<extern "C" fn(time: i64, out: *mut YVR_EyeTrackingData) -> f64> = {
            ...
            let handle = unsafe {
                dlopen(libtrackingclient.as_ptr(), RTLD_NOW)
            };
            if !handle.is_null() {
                let fnptr = unsafe {
                ...
                if !fnptr.is_null() {
                    ...
                    unsafe {
                        let fn_getapi: extern "C" fn(version: c_uint, out: *mut *mut c_void) -> f64 = std::mem::transmute(fnptr);
                        fn_getapi(0x2710, &mut apitable as *mut *mut c_void);
                    }

                    if !apitable.is_null() {
                        unsafe {
                            let func_table: &[*const c_void] = slice::from_raw_parts(apitable as *const *const c_void, 0x110);
                            if func_table[71].is_null() {
                                None
                            }
                            else {
                                let fn_GetEyeDataET: extern "C" fn(time: i64, out: *mut YVR_EyeTrackingData) -> f64 = std::mem::transmute(func_table[71]);
        ...
        Ok(Self { handle, ext_fns, fn_GetEyeDataET })
    }
        ...
        if let Some(fn_GetEyeDataET) = self.fn_GetEyeDataET
        {
            ...
            if real_left_valid && real_right_valid {
                ...
                if denom.abs() < 1e-6 {
                    ...
                    unsafe { EYE_CONVERGENCE = smoothed_depth; }
